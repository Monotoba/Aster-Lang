...
from aster_lang.bytecode import program_to_json
...
def build_project_vm(
    *,
    ...
    result.entry_py.write_text(_render_vm_launcher(program_path.name), encoding="utf-8")
    return result


def _render_vm_launcher(program_filename: str) -> str:
    return "\n".join(
        [
